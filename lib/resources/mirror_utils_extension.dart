...
extension MirrorGetExtension on Mirror {
  ...
  void addStack({required int stackSize, bool? test}) {
    ...
    for(VocabEntry entry in stack) {
      ...
      undo.addFunction(() => writeEntry(entry: entryCopy, test: test));
      move(entry: entry, direction: Direction.next, test: test, addNewUndo: false);
    }
    addUndo(undo: undo);
  }

  // Move in direction enum Directions next / previous / first
  void move({required VocabEntry entry, required Direction direction, bool ? test, required bool addNewUndo}) {
    ...
    switch (direction) {
      case Direction.next:
        ...
        writeEntry(entry: entry, test: test);
        break;
