...
void main() {
    ...
    test('writeEntry to DatabaseMirror (without updating sql database)', () {
      ...
      VocabEntry entryClone = Mirror().writeEntry(entry: entry, test: true);
      ...
      // Add second entry
      entry.vocabKey = 2;
      VocabEntry entryClone2 = Mirror().writeEntry(entry: entry, test: true);
    ...
    test('delete entry from DatabaseMirror', () {
      VocabEntry entry = VocabEntry(
          ...
          timeModified: 1);
      Mirror().dbMirror.clear();
      bool deleted = Mirror().deleteEntry(vocabKey: 0, test: true);
      ...
      entry.vocabKey = 3;
      Mirror().writeEntry(entry: entry, test: true);
      deleted = Mirror().deleteEntry(vocabKey: 2, test: true);
      expect(Mirror().dbMirror.length, 2);
      expect(Mirror().dbMirror[0].vocabKey, 1);
